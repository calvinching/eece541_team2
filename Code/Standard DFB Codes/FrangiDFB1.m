function [outIm,whatScale,Direction] = FrangiDFB1(I, options)
...
defaultoptions = struct('FrangiScaleRange', [1 10], 'FrangiScaleRatio', 2, 'FrangiBetaOne', 0.5, 'FrangiBetaTwo', 5, 'verbose',true,'BlackWhite',true);
...
oimg=DFB_based_orientations(I); %Directions for each pixel
oimg=-oimg; % rotate the directions
...
for i = 1:length(sigmas),
    ...
    mu1band = ((cos(oimg)).^2).*Dxx + sin(2*oimg).*Dxy...
                    + (sin(oimg).^2).*Dyy;
    mu2band = ((sin(oimg)).^2).*Dxx - sin(2*oimg).*Dxy...
                    + (cos(oimg).^2).*Dyy;
